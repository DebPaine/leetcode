...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        return new_head   # we return the new head which is 5 (the last node) in every recursive call since we want to pass the new head from the top of the call stack (when we reach the last node) till the bottom of the stack (when we reach the first node) so that we can get the starting node of this reversed linked list