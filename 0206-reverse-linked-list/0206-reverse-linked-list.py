...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        from the top of the call stack (when we reach the last node) till the bottom of the stack (when we reach the first node) so that we can get the starting node of this reversed linked list
        """ 
        return new_head   